...
public class GetTrackFromUrl extends AsyncTask<String, Void, ArrayList<Track>> {
    private TrackDataSource.RemoteDataSource.OnFetchDataListener mOnFetchDataListener;
    private Exception mException;

    public GetTrackFromUrl(TrackDataSource.RemoteDataSource.OnFetchDataListener onFetchDataListener) {
    ...
    public ArrayList<Track> getTrackFromJSon(String data) throws JSONException {
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
            JSONObject jsonObject = jsonArray.getJSONObject(i).getJSONObject(Track.JsonKey.TRACK);
